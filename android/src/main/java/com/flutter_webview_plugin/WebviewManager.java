...
import com.flutter_webview_plugin.helper.DataHelper;
import com.flutter_webview_plugin.helper.DownloadHelper;
import com.flutter_webview_plugin.helper.ResAutoCacheHelper;
...
class WebviewManager {
    ...
    private ResAutoCacheHelper resCacheHelper;
    ...
    WebviewManager(final Activity activity, final Context context, final List<String> channelNames) {
        ...
        resCacheHelper = new ResAutoCacheHelper(context);
        ...
        TimerTask task = new TimerTask() {
            public void run() {
                if (queue.size() != 0) {
                    try {
                        String downloadUrl = queue.poll();
                        Log.e("Queue", "Queue處理=>" + downloadUrl);
                        String[] arr = downloadUrl.split("/");
                        String parentPath = arr[arr.length - 2];
                        String fileName = arr[arr.length - 1];

                        downloadHelper.downloadFile(downloadUrl, parentPath, fileName,autoDownloadResource);
        ...
        webViewClient = new BrowserClient(null) {
            @Override
...
//        Log.e("BrowserClient", "shouldInterceptRequest=>" + url);
                if (mDataHelper.hasLocalResource(url)) {
                    Log.e("BrowserClient", "have local file=>" + url);
                    WebResourceResponse response = mDataHelper.getReplacedWebResourceResponse(context, url);
                    if (response != null) {
                        return response;
                    }
                } else {
                    if (resCacheHelper.hasLocalResource(url)) {
                        Log.e("resCache", "HAVE RES");
                        WebResourceResponse response = resCacheHelper.getReplacedWebResourceResponse(url);
                        if (response != null) {
                            Log.e("resCache", "SUCCESS GOT CACHE");
                            return response;
                        }
                    } else {
                        addToSaved(url);
            ...
            private void addToSaved(String url) {
...
//        }
    ...
    void autoDownloadResource(boolean isOn){
        this.autoDownloadResource = isOn;
    }
    ...
    void setCookie(String url, List<Map<String, String>> cookie) {
        ...
        for (int i = 0; i < cookie.size(); i++) {
            Map<String, String> map = cookie.get(i);
